...
async function main() {
  ...
  `);
    ...
    const sendSmall = await agent.executeCommand(agent.parseCommand("send 0.001 ETH to 0x742d35Cc6634C0532925a3b844Bc9e7595f8f44"));
    console.log(sendSmall);
    await delay(300);
    console.log(`\n  ${c("yellow", "→")} Attempting large transfer (exceeds single tx limit)...`);
    const sendLarge = await agent.executeCommand(agent.parseCommand("send 50 ETH to 0x742d35Cc6634C0532925a3b844Bc9e7595f8f44"));
    ...
    const activate = await agent.executeCommand(agent.parseCommand("strategy activate dca"));
