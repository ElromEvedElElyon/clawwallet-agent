...
export class SafetyGuard {
  ...
  checkTransaction(req: TransactionRequest, estimatedUsd: number): {
    ...
    const needsConfirmation =
      estimatedUsd > this.config.requireConfirmationAboveUsd &&
  ...
  private log(action: string, chain: string, details: string, amountUsd: number, approved: boolean): void {
    this.auditLog.push({
      timestamp: Date.now(),
      action,
      chain,
  ...
  getDailyStats(): { total: number; remaining: number; date: string } {
    this.resetDailyIfNeeded();
    return {
