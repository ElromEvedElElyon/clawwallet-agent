...
export class ClawWalletAgent {
  ...
  async executeCommand(cmd: AgentCommand): Promise<string> {
    switch (cmd.intent) {
      case "balance":
  ...
  private async handleBalance(chain?: string): Promise<string> {
    if (chain) {
  ...
  private async handlePortfolio(): Promise<string> {
    const balances = await this.wallets.getAllBalances();
  ...
  private async handleSend(cmd: AgentCommand): Promise<string> {
    ...
    const check = this.safety.checkTransaction(
      { chain, to: cmd.to, amount: cmd.amount },
      estimatedUsd,
    );
  ...
  private async handleSwap(_cmd: AgentCommand): Promise<string> {
  ...
  private async handleStrategy(cmd: AgentCommand): Promise<string> {
    const strategies = this.strategies.getStrategies();
    ...
    if (cmd.raw.includes("activate")) {
      ...
      const found = strategies.find(
        (s) => s.name.toLowerCase().includes(name) || name.includes(s.type),
      );
      if (found) {
        await this.strategies.activateStrategy(
          Array.from(this.strategies.getStrategies()).indexOf(found) === 0
            ? "dca-eth"
...
`;
  ...
  private inferChain(token?: string): string {
    if (!token) return "ethereum";
    const t = token.toLowerCase();
    if (t === "btc" || t === "bitcoin") return "bitcoin";
    if (t === "sol" || t === "solana") return "solana";
    if (t === "matic" || t === "polygon") return "polygon";
    if (t === "trx" || t === "tron") return "tron";
    return "ethereum";
  }

  private estimateUsd(amount: string, token: string): number {
